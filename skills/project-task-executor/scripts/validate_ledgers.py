...
REQUIRED_EVENT_FILE = "AGENT_EVENTS.jsonl"
...
def validate_event_log(path: Path) -> list[Issue]:
    if not path.exists():
        return [Issue(path, 1, "missing required event log")]
    text = path.read_text(encoding="utf-8")
    if not text.strip():
        return [Issue(path, 1, "event log is empty")]
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            issues.append(Issue(path, lineno, f"invalid JSONL event: {exc.msg}"))
            continue
        if not isinstance(event, dict):
            issues.append(Issue(path, lineno, "event must be a JSON object"))
            continue
        if not isinstance(event.get("ts"), str) or not event["ts"]:
            issues.append(Issue(path, lineno, "event missing non-empty string 'ts'"))
        if not isinstance(event.get("event"), str) or not event["event"]:
            issues.append(Issue(path, lineno, "event missing non-empty string 'event'"))
    return issues
...
def parse_review_verdict(review_path: Path) -> tuple[str | None, list[Issue]]:
    if not review_path.exists():
        return None, [Issue(review_path, 1, "missing required ledger file")]

    lines = review_path.read_text(encoding="utf-8").splitlines()
    matches: list[tuple[int, str]] = []
    for lineno, line in enumerate(lines, start=1):
        match = re.match(r"^Verdict:\s*(?P<verdict>\S+)\s*$", line.strip())
        if match:
            matches.append((lineno, match.group("verdict")))
    ...
    if not matches:
        return None, [
            Issue(
                review_path,
                1,
                "AGENT_REVIEW.md missing required 'Verdict: APPROVE | REQUEST_CHANGES | BLOCKED'",
            )
        ]

    issues: list[Issue] = []
    if len(matches) > 1:
        for lineno, _ in matches[1:]:
            issues.append(Issue(review_path, lineno, "AGENT_REVIEW.md should contain exactly one Verdict line"))

    line, verdict = matches[0]
    if verdict not in ALLOWED_REVIEW_VERDICTS:
        allowed = " | ".join(sorted(ALLOWED_REVIEW_VERDICTS))
        issues.append(Issue(review_path, line, f"unknown Verdict '{verdict}' (allowed: {allowed})"))

    return verdict, issues


def validate_evidence_pack(review_path: Path) -> list[Issue]:
    text = review_path.read_text(encoding="utf-8") if review_path.exists() else ""
    issues: list[Issue] = []

    if not re.search(r"^##\s+Evidence Pack\s*$", text, re.MULTILINE):
        issues.append(Issue(review_path, 1, "AGENT_REVIEW.md missing required '## Evidence Pack' section"))

    for field in EVIDENCE_PACK_FIELDS:
        match = re.search(rf"^\s*[-*]\s*{re.escape(field)}:\s*(?P<value>.*)$", text, re.MULTILINE)
        if not match or not match.group("value").strip():
            issues.append(Issue(review_path, 1, f"Evidence Pack missing non-empty '{field}:'"))
...
def validate_review(review_path: Path, *, require_complete: bool = False) -> list[Issue]:
    verdict, issues = parse_review_verdict(review_path)

    if require_complete:
        if verdict != "APPROVE":
            issues.append(Issue(review_path, 1, "Verdict must be APPROVE before completion"))
        issues.extend(validate_evidence_pack(review_path))
...
def validate(root: Path, *, require_executable_plan: bool = False, require_complete: bool = False) -> list[Issue]:
    ...
    issues.extend(validate_event_log(root / REQUIRED_EVENT_FILE))

    review_path = root / "AGENT_REVIEW.md"
    if review_path.exists() and review_path.read_text(encoding="utf-8").strip():
        issues.extend(validate_review(review_path, require_complete=require_complete))
